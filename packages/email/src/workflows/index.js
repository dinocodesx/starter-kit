import { createEmailDeliveryRecord, markEmailDeliveryFailed, markEmailDeliverySent, } from "@creator-suite/db";
import { randomUUID } from "node:crypto";
import { createResendClient } from "../senders/index.js";
import { renderTrialEndingEmail, renderUpgradeEmail, renderWelcomeEmail, } from "../templates/index.js";
function normalizeRecipients(to) {
    return Array.isArray(to) ? to : [to];
}
function buildDeliveryKey(template) {
...
export function createEmailService({ prisma, resendApiKey, fromAddress = "Onboarding <onboarding@resend.dev>", appName = "your workspace", }) {
    ...
    async function sendEmail(input) {
        ...
        try {
            ...
            if (error || !data?.id) {
                throw new Error(stringifyError(error ?? new Error("Resend did not return an email id.")));
            }
    ...
    async function sendUpgradeEmail(message, to) {
    ...
    async function sendTrialEndingEmail(message, to) {
